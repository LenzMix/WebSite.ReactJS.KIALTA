...
class ClientRoomsPage extends React.Component {
    constructor(props) {
        ...
        this.MeAdmin=this.MeAdmin.bind(this);
        this.socketusrs=this.socketusrs.bind(this);
    }
    ...
    socketusrs(){
        let listMess = this.state.usrs.map((info,i) =>
        <tr key={i}>
            <td>
            {info.id}
            </td>  
    ...
    socketListener(){
        ...
        socket.removeAllListeners('userclear');
        socket.removeAllListeners('userrebuild');
        socket.on('useradd', (rid,ruserName,rsession,rperm) => {
            console.log('Добавлен пользователь: '+ruserName);
            let us = {
                id: rid,
                userName: ruserName,
                session: rsession,
        ...
        socket.on('userclear', () => {
            console.log('Пользователи отчищены');
            users = [];
        });
        socket.on('userrebuild', () => {
            console.log('Пользователи занесены в стейт');
            this.setState({usrs: users});
        });*/
        console.log('Начинаю прослушку');
        socket.removeAllListeners('useradd');
        socket.removeAllListeners('userclear');
        socket.removeAllListeners('userrebuild');
        socket.on('userclear', () => {
        ...
        socket.on('useradd', (rid,ruserName,rsession,rperm) => {
            ...
            console.log(this.state.usrs,' lawyers' );
            this.setState({
                usrs: this.state.usrs
            })
        });
        socket.on('userrebuild', () => {
            
            console.log(this.state.usrs,'lawyersended');
