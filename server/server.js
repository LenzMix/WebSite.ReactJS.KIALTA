...
function SessionUP() {
    for (let i = 0; i < users.length; i++) {
        if (users[i].permission > 0) {  
            users[i].sessiontime = users[i].sessiontime+1;
            db.query("UPDATE user_info SET sessiontime=$1 WHERE id=$2",[users[i].sessiontime,users[i].urid],function(err,result){
...
io.on('connection', function(socket) {
            ...
            console.error('ЕБАТЬ КАКАЯ ТО ОШИБКА', err);
                ...
                for (let i = 0; i < sqlinfo.rowCount; i++) {
                    ...
                    for (let g = 0; g < users.length; g++) {
                        if (users[g].urid == sqlinfo.rows[i].id) isOnline = true;
                    }
                    socket.emit('layeradd',sqlinfo.rows[i].fio,sqlinfo.rows[i].description,sqlinfo.rows[i].prof_img,sqlinfo.rows[i].benches,sqlinfo.rows[i].exp,sqlinfo.rows[i].atts_imgs,isOnline);
    ...
    socket.on('needusr',function(){
        socket.emit('userclear');
        console.log('Запрос на .cthjd');
        db.query('SELECT * FROM user_info',[],function (err,sqlinfo) {
            if (err) {
                console.error('ЕБАТЬ КАКАЯ ТО ОШИБКА', err);
            }
            else {
                for (let i = 0; i < sqlinfo.rowCount; i++) {
                    socket.emit('useradd',sqlinfo.rows[i].id,sqlinfo.rows[i].fio,sqlinfo.rows[i].sessiontime,sqlinfo.rows[i].exp,sqlinfo.rows[i].perm);
                }
            }
        });
        socket.emit('userrebuild');
        console.log('Запрос на подгрузку фурри');
    });
    /* Получает клиент
    ...
    */
                        ...
                        db.query('SELECT * FROM user_info WHERE id=$1',[ret.rows[0].id],function (err,sqlinfo) {
                            ...
                            console.error('ЕБАТЬ КАКАЯ ТО ОШИБКА', err);
                                ...
                                if (mynumb != null) {
                                    ...
                                    mynumb.sessiontime = sqlinfo.rows[0].sessiontime;
                                    SetMe(socket,mynumb);
                                    newinfo = FindMe(socket);
            ...
            db.query('SELECT * FROM user_info WHERE perm=1',[],function (err,sqlinfo) {
                ...
                console.error('ЕБАТЬ КАКАЯ ТО ОШИБКА', err);
                    ...
                    for (let i = 0; i < sqlinfo.rowCount; i++) {
                        ...
                        socket.emit('layeradd',sqlinfo.rows[i].fio,sqlinfo.rows[i].description,sqlinfo.rows[i].prof_img,sqlinfo.rows[i].benches,sqlinfo.rows[i].exp,sqlinfo.rows[i].atts_imgs);
                        console.log('Добавляю новую фурри',sqlinfo.rows[i].fio,sqlinfo.rows[i].description,sqlinfo.rows[i].prof_img,sqlinfo.rows[i].benches,sqlinfo.rows[i].exp);
                    }
                }
            });
    ...
    socket.on('msgtosrv', function(rwho,rname,rmes){ 
        ...
        if (mynumb.permission > 0 && mynumb != null) {
            ...
            socket.emit('errorjoin','Ошибка отправки');
    ...
    socket.on('edituser',function(id,pass,fio,description,benches,exp,prof_img){ 
        ...
        if (mynumb.permission > 1) {
                    ...
                    if (description!=='')   
                    db.query("UPDATE user_info SET description=$1 WHERE ID=$2",[description,id],function(err,result){
                        if(err){
                            socket.emit('errorjoin','При редактировании таблицы с информацией произошла ошибка');
                        }
                        else{
